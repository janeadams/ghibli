# Thanks Hayden Faulkner: https://medium.com/@haydenfaulkner/extracting-frames-fast-from-a-video-using-opencv-and-python-73b9b7dc9661
...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1, film=''):
    """
    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            save_path = os.path.join(frames_dir, video_filename, film+"_{:010d}.jpg".format(frame))  # create the save path
...
def video_to_frames(video_path, frames_dir, overwrite=False, every=1, chunk_size=1000, film=''):
    """
    ...
    with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
        ...
        futures = [executor.submit(extract_frames, video_path, frames_dir, overwrite, f[0], f[1], every, film)
